...
Sources (enable/disable and tune in config.json):
    item_master     any CSV/XLSX you upload to inbox/ (runs within minutes of the upload)
...
def read_table(path):
    """Return (header, rows) from a CSV/TXT or XLSX file. Values are strings."""
    if path.suffix.lower() == ".xlsx":
        from openpyxl import load_workbook
        wb = load_workbook(path, read_only=True, data_only=True)
        for ws in wb.worksheets:                       # first sheet that has a barcode column
            it = ws.iter_rows(values_only=True)
            for header in it:
                if header and any(v not in (None, "") for v in header):
                    break
            else:
                continue
            header = [str(h or "").strip() for h in header]
            if not any(BARCODE_COL.search(h) for h in header):
                continue
            ...
            def cell(v):
                ...
                if isinstance(v, float) and v.is_integer():
                    return str(int(v))                 # 3.66014091086e12 -> 3660140910859
                return str(v)
            return header, ([cell(v) for v in r] for r in it)
        return [], iter(())
...
def src_item_master(db, st, cfg):
    ...
    if not files:
        ...
        return True
    report = [f"Inbox import report {db.ts}", ""]
    queue = []
    for f in files:
        digest = hashlib.sha256(f.read_bytes()).hexdigest()
        if st.get("hashes", {}).get(f.name) == digest:
            log(f"{f.name} unchanged since last import")
            continue
        try:
            header, rows = read_table(f)
        except Exception as e:
            report.append(f"{f.name}: could not be read ({e}). Save it as .xlsx or .csv and upload again.")
            continue
        ...
        n = bad = nodesc = same = 0
        for r in rows:
            if bi >= len(r) or not str(r[bi]).strip():
                continue
            n += 1
            code = str(r[bi]).strip()
            ean = to_ean13(code, True)
            if not ean:
                bad += 1
                continue
            desc = str(r[di]).strip() if di is not None and di < len(r) else ""
            if len(clean(desc)) < 3:
                nodesc += 1
                if ean not in db.rows:
                    queue.append(ean)           # no description: look it up online instead
                continue
            if not db.upsert(ean, desc):
                same += 1
        a = db.stats["item_master"]["added"] - before["added"]
        u = db.stats["item_master"]["updated"] - before["updated"]
        report.append(f"{f.name}: {n:,} rows, {a:,} added, {u:,} updated, {same:,} unchanged or kept, "
                      f"{bad:,} invalid barcodes, {nodesc:,} without description"
                      + (f" (description column: '{header[di]}')" if di is not None else " (no description column found)"))
        st.setdefault("hashes", {})[f.name] = digest
    if queue:
        mf = INBOX / "missing_barcodes.txt"
        old = mf.read_text(encoding="utf-8").split() if mf.exists() else []
        merged = list(dict.fromkeys([x for x in old if x.isdigit()] + queue))
        mf.write_text("\n".join(merged) + "\n", encoding="utf-8")
        report.append(f"{len(set(queue)):,} barcodes without a description were added to missing_barcodes.txt for online lookup.")
    if len(report) > 2:
        (INBOX / "import_report.txt").write_text("\n".join(report) + "\n", encoding="utf-8")
